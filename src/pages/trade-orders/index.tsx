...
import { getTradeOrders, deleteTradeOrderById } from 'apiSdk/trade-orders';
import { TradeOrderInterface } from 'interfaces/trade-order';

type ColumnType = ColumnDef<TradeOrderInterface, unknown>;

interface TradeOrderListPageProps {
...
export function TradeOrderListPage(props: TradeOrderListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TradeOrderInterface>>(
    () => `/trade-orders?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TradeOrderInterface) => {
    if (hasAccess('trade_order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/trade-orders/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'order_type', header: 'Order Type', accessorKey: 'order_type' },
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/trade-orders/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('trade_order', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/trade-orders/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('trade_order', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="trade_order" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Trade Orders
        </Text>
        {hasAccess('trade_order', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/trade-orders/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
