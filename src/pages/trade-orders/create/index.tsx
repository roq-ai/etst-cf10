...
import { createTradeOrder } from 'apiSdk/trade-orders';
...
function TradeOrderCreatePage() {
  const router = useRouter();
  const [error, setError] = useState(null);

  const handleSubmit = async (values: TradeOrderInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createTradeOrder(values);
      resetForm();
  ...
  const formik = useFormik<TradeOrderInterface>({
    initialValues: {
      order_type: '',
      quantity: 0,
      price: 0,
    ...
    validationSchema: tradeOrderValidationSchema,
